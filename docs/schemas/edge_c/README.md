...
**范围**：只承载 `GET_DEVICE_INFO` 的稳定结果字段，不承载任务生命周期相位；生命周期仍由 `motion_event` 表达。
...
| `type` | string | 是 | 固定为 `device_info` |
| `task_id` | string | 是 | 与下行 `motion_task` 的 `task_id` 一致 |
| `device_id` | string | 是 | 下行任务体携带的设备 ID |
| `capability` | string | 否 | 原始能力名字符串（通常为 `get_device_info`） |
| `model` | string | 是 | 设备型号 |
| `hw_rev` | string | 是 | 硬件版本 |
| `fw_rev` | string | 是 | U1 固件版本 |
| `workspace_mm` | object | 是 | 工作区毫米尺寸，含 `x/y/z` |

示例见 `examples/device_info.uplink.json`。

### DeviceServer → BusinessServer（HTTP）

当且仅当 `xiaozhi-server` 配置中 **`server.device_info_business_base_url`**（为空时复用 `server.motion_event_business_base_url`）与 **`server.internal_motion_task_token`** 同时非空：

- **方法 / 路径**：`POST {base}/internal/v1/device_info`
...
| `task_id` | string | 是 | 与下行 `motion_task` 的 `task_id` 一致 |
| `phase` | string | 是 | `accepted` \| `running` \| `progress` \| `done` \| `failed` \| `cancelled` \| `rejected` |
| `device_id` | string | 否 | 若下行任务体携带则回显，便于 DeviceServer 路由 |
| `capability` | string | 否 | 原始能力名字符串（与下行一致） |
| `source` | string | 否 | `client` 或 `voice`，用于 TTS 策略 |
| `progress` | object | 否 | `phase=progress` 时携带 `done_segments / total_segments / percent` |

示例见 `examples/motion_event.minimal.uplink.json` 与 `examples/motion_event.progress.uplink.json`。
